...
if (isset($_POST['login'])) {
  ...
  $stmt = $mysqli->prepare("SELECT admin_email, admin_password, admin_id FROM rpos_admin WHERE admin_email =?"); //sql to log in user
  $stmt->bind_param('s',  $user_email); //bind fetched parameters
  $stmt->execute(); //execute bind 
  $stmt->bind_result($admin_email, $admin_password, $admin_id); //bind result
  $rs = $stmt->fetch();
  //$_SESSION['admin_id'] = $admin_id;
  if ($rs && password_verify($user_password, $admin_password)) {
    //if its sucessfull
    $_SESSION['admin_id'] = $admin_id;
